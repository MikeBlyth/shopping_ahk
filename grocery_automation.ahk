...
; GUI Functions for user interaction
ShowItemPrompt(param) {
    ...
    if is_known {
        ...
        message .= "`nReady to add to cart. What would you like to do?"
    } else {
        message := "🔍 New item: " . item_name . "`n"
        message .= "Search results should be displayed. Find the correct product and add to cart.`n"
        message .= "`nWhat would you like to do next?"
    }
    
    ; Create custom GUI with proper button labels
    gui := Gui("+AlwaysOnTop", "Item: " . item_name)
    gui.Add("Text", "w400", message)
    gui.Add("Text", "w400 y+10", "Choose what to do next:")
    
    continueBtn := gui.Add("Button", "w120 y+10", "&Continue")
    saveBtn := gui.Add("Button", "w120 x+10 yp", "&Save URL")
    priceBtn := gui.Add("Button", "w120 x+10 yp", "Record &Price")
    
    response := ""
    continueBtn.OnEvent("Click", (*) => (response := "continue", gui.Close()))
    saveBtn.OnEvent("Click", (*) => (response := "save_url", gui.Close()))
    priceBtn.OnEvent("Click", (*) => (response := "record_price", gui.Close()))
    
    gui.Show()
    
    ; Wait for user choice
    while gui.Hwnd && response = ""
        Sleep(100)
    
    WriteResponse(response != "" ? response : "continue")
...
GetPriceInput() {
    ...
    price := InputBox("💰 Enter the current price (e.g., 3.45):", "Record Price", "w300 h150").Text
    
    if price = "" {
        WriteResponse("cancelled")
    } else {
        WriteResponse("price|" . price)
    }
